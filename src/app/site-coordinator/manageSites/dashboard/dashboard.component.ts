...
export class DashboardComponent implements OnInit {
  ...
  getstudiesWithSite() {
    ...
    this.errorMessage = "";
    this.manageSiteService.getstudiesWithSite().subscribe(
      (data) => {
        this.studiesWithSites = data;
        this.studiesWithSitesBackup = JSON.parse(
          JSON.stringify(this.studiesWithSites)
        );
      },
      (error) => {
        this.studiesWithSites = [];
  ...
  getStudies() {
    ...
    this.studiesBackup = [];
    this.errorMessage = "";
    this.manageSiteService.getStudies().subscribe(
      (data) => {
        this.studies = data;
  ...
  getApps() {
    ...
    this.appsBackup = [];
    this.errorMessage = "";
    this.manageSiteService.getApps().subscribe(
      (data) => {
        this.apps = data;
  ...
  addSiteData(template) {
    ...
    this.manageSiteService.addSite(this.addSite).subscribe(
      (data) => {
  ...
  search(filterQuery) {
    ...
    if (this.activeTab == "sites") {
      if (query && query.trim() != "" && query.trim() != undefined) {
        this.studiesWithSites = this.studiesWithSitesBackup.filter(function (
          a
        ) {
          return a.sites.some(function (b) {
            return (
              (b.customId != null &&
                b.customId != undefined &&
                b.customId.toLowerCase().includes(query.toLowerCase())) ||
              (b.name != null &&
                b.name != undefined &&
              ...
              (a.customId != null &&
                a.customId != undefined &&
                a.customId.toLowerCase().includes(query.toLowerCase()))
            );
          });
        });
      } else {

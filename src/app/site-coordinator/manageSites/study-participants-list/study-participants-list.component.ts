...
export class StudyParticipantsListComponent implements OnInit {
  ...
  getstudiesRegistryDetails() {
    this.manageSitesService
      .getstudiesParticipantRegistry(this.studyId)
      .subscribe(
        (data) => {
          console.log(data);
          this.studyDetails = data;
          this.participantRegistryListBackup = JSON.parse(
            JSON.stringify(this.studyDetails.registryParticipants)
          );
        },
        (error) => {
  ...
  search(filterQuery) {
    ...
    if (query && query.trim() != "" && query.trim() != undefined) {
      this.studyDetails.registryParticipants = this.participantRegistryListBackup.filter(
        function (a) {
          return (
            (a.email != null &&
              a.email != undefined &&
